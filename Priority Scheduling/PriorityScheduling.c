...
int comparePriorityTime(const void  *s1, const void *s2)
{
  ...
  if(e1->arivalTime <= currentTime && e2->arivalTime <= currentTime) {
    if(e1->priorityNumber == e2->priorityNumber) return e1->arivalTime > e2->arivalTime;
    else return e1->priorityNumber > e2->priorityNumber;
  }
...
void SortProcesses() {
    for(int i=0;i<numProcesses;i++)
    {
        for(int j=i+1;j<numProcesses;j++){
            if(comparePriorityTime(&Process[i], &Process[j])){
